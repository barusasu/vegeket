...
class User(AbstractBaseUser):
    ...
    def has_module_perms(self, app_label):
        "Does the user habe permission to view the app 'app_label'?"
        # Simplest possible answer: Yes, always
        return True
...
class Profile(models.Model):
    user = models.OneToOneField(User, primary_key=True, on_delete=models.CASCADE)   # 1対1で紐づくからOneToOneFieldを使う。Userが削除されたらProfileも削除するためにCASCADE
...
# OneToOneFieldを同時に作成
@receiver(post_save, sender=User)   # Userモデルが保存されるとき(post_save)に同時にProfileモデルを作成する
